...
import { addCategory, archiveCategory, moveCategory, updateCategory } from '../db/repository';
...
function CategorySection({ type, categories }: { type: EntryType; categories: readonly Category[] }) {
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <CategorySection type="expense" categories={categories} />
        <CategorySection type="income" categories={categories} />
      </div>
    </div>
  );
}
